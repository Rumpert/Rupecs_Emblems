...
public class EntherealSelectorRecipeBuilder {
    ...
    public EntherealSelectorRecipeBuilder(Item result, int count) {
        this.result = result;
        this.resultCount = count;
    }

    public static EntherealSelectorRecipeBuilder etherealSelection(Item result, int count) {
        return new EntherealSelectorRecipeBuilder(result, count);
    }

    // Simple ingredient without NBT conditions (item)
    public EntherealSelectorRecipeBuilder requires(Item item, int count) {
        ingredientsWithNbt.add(new IngredientWithNbt(Ingredient.of(item), count, null));
        return this;
    }

    public EntherealSelectorRecipeBuilder requires(Item item) {
        return requires(item, 1);
    }

    // Simple ingredient without NBT conditions (tag)
    public EntherealSelectorRecipeBuilder requires(TagKey<Item> tag, int count) {
        ingredientsWithNbt.add(new IngredientWithNbt(Ingredient.of(tag), count, null));
        return this;
    }

    public EntherealSelectorRecipeBuilder requires(TagKey<Item> tag) {
        return requires(tag, 1);
    }

    // Simple ingredient without NBT conditions (custom ingredient)
    public EntherealSelectorRecipeBuilder requires(Ingredient ingredient, int count) {
        ingredientsWithNbt.add(new IngredientWithNbt(ingredient, count, null));
        return this;
    }

    // Ingredient with NBT conditions (via lambda)
    public EntherealSelectorRecipeBuilder requires(Item item, int count, Consumer<IngredientNbtBuilder> nbtBuilder) {
    ...
    public EntherealSelectorRecipeBuilder requires(Item item, Consumer<IngredientNbtBuilder> nbtBuilder) {
        return requires(item, 1, nbtBuilder);
    }

    // For tag with NBT conditions (if needed)
    public EntherealSelectorRecipeBuilder requires(TagKey<Item> tag, int count, Consumer<IngredientNbtBuilder> nbtBuilder) {
        IngredientNbtBuilder builder = new IngredientNbtBuilder();
        nbtBuilder.accept(builder);
    ...
    public EntherealSelectorRecipeBuilder requires(TagKey<Item> tag, Consumer<IngredientNbtBuilder> nbtBuilder) {
        return requires(tag, 1, nbtBuilder);
    }

    // For custom Ingredient with NBT conditions
    public EntherealSelectorRecipeBuilder requires(Ingredient ingredient, int count, Consumer<IngredientNbtBuilder> nbtBuilder) {
        IngredientNbtBuilder builder = new IngredientNbtBuilder();
        nbtBuilder.accept(builder);
    ...
    public EntherealSelectorRecipeBuilder withNbt(JsonObject nbt) {
        this.resultNbt = nbt;
        return this;
    }

    public EntherealSelectorRecipeBuilder withIntTag(String key, int value) {
    ...
    public static class Result implements FinishedRecipe {
        ...
        public RecipeSerializer<?> getType() {
            return ModRecipes.ENTHEREAL_SELECTOR_SERIALIZER.get();
        }
