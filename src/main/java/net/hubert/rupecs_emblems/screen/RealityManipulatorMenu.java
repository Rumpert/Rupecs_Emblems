...
public class RealityManipulatorMenu extends AbstractContainerMenu {
    public final RealityManipulatorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;


    public RealityManipulatorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData){
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public RealityManipulatorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.REALITY_MANIPULATOR_MENU.get(), pContainerId);
        checkContainerSize(inv, 4);
        blockEntity = ((RealityManipulatorBlockEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.REALITY_MANIPULATOR.get());
    }
    ...
    public float getCraftChance() {
        float bonusLuck = 1;
        if (level != null){
            Player player = level.getNearestPlayer(this.blockEntity.getBlockPos().getX(), this.blockEntity.getBlockPos().getY(), this.blockEntity.getBlockPos().getZ(), 10, false);
            if (player != null) {
                bonusLuck += Math.min(((float) player.getAttributeValue(ModAttributes.LUCKY.get()) / 10), 0.35f);
            }
        }

        return data.get(2) * bonusLuck;
    }
}
