...
public class VolcanicShardRender extends EntityRenderer<VolcanicShardProjectile> {

    private static final ResourceLocation COMET_SHARD_TEXTURE = new ResourceLocation(Rupecs_Emblems.MOD_ID, "textures/entity/volcanic_shard.png");

    public VolcanicShardRender(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(VolcanicShardProjectile entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
        ...
        VertexConsumer vertexConsumer = buffer.getBuffer(RenderType.entityCutout(COMET_SHARD_TEXTURE));
    ...
    @Override
    public ResourceLocation getTextureLocation(VolcanicShardProjectile entity) {
        return COMET_SHARD_TEXTURE;
    }
